...
function mergeObjects(objects) {
  const dest = { ...objects[0] };
  if (objects.length > 1) {
    objects.forEach((obj) => {
      Object.entries(obj).reduce((acc, [key, value]) => {
        acc[key] = key in acc && acc[key] !== value ? acc[key] + value : value;
        return acc;
      }, dest);
    });
  }
...
function compareObjects(obj1, obj2) {
  return Object.entries(obj1).every(
    ([key, value]) => key in obj2 && obj2[key] === value
  );
}
...
function sellTickets(queue) {
  ...
  let hasChange = true;
  queue.forEach((bill) => {
    const change = bill > 25 ? bill - 25 : 0;
    switch (change) {
      case 25:
        if (cashier[25] > 0) {
          cashier[25] -= 1;
...
class SelectorBuilder {
  ...
  checkOrder(type) {
    ...
    if (this.order.some((o) => order.indexOf(o) > index)) {
      throw new Error(
